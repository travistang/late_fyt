...
class SumTree(object):
    ...
    def _find(self, value, index):
        if 2**(self.tree_level-1)-1 <= index:
            return self.data[int(index-(2**(self.tree_level-1)-1))], self.tree[int(index)], index-(2**(self.tree_level-1)-1)
